...
#include "CNMSSMEFTHiggs_physical.hpp"
#include "CNMSSMEFTHiggs_slha_io.hpp"
...
namespace CNMSSMEFTHiggs_librarylink {
...
private:
   ...
   void flush() {
      ...
      while (std::getline(buffer, line)) {
         ...
         MLPutFunction(link, "FSCNMSSMEFTHiggsMessage", 1);
...
class CNMSSMEFTHiggs_spectrum {
public:
   virtual ~CNMSSMEFTHiggs_spectrum() = default;

   virtual void put_model_spectra(MLINK link) const = 0;

   virtual const Spectrum_generator_problems& get_problems() const = 0;
   virtual void fill_slha_io(CNMSSMEFTHiggs_slha_io&) const = 0;
   virtual double get_model_scale() const = 0;
   virtual const CNMSSMEFTHiggs_observables& get_observables() const = 0;

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CNMSSMEFTHiggs_input_parameters&) = 0;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) = 0;
};

template <typename Solver_type>
class CNMSSMEFTHiggs_spectrum_impl : public CNMSSMEFTHiggs_spectrum
{
public:
   virtual ~CNMSSMEFTHiggs_spectrum_impl() = default;
   ...
   virtual void fill_slha_io(CNMSSMEFTHiggs_slha_io&) const override;
   virtual double get_model_scale() const override { return std::get<0>(models).get_scale(); }
   virtual const CNMSSMEFTHiggs_observables& get_observables() const override { return observables; }

   virtual void calculate_spectrum(
      const Spectrum_generator_settings&, const SLHA_io::Modsel&,
      const softsusy::QedQcd&, const CNMSSMEFTHiggs_input_parameters&) override;
   virtual void calculate_model_observables(const softsusy::QedQcd&, const Physical_input&) override;

private:
   std::tuple<CNMSSMEFTHiggs_slha<CNMSSMEFTHiggs<Solver_type>>, standard_model::StandardModel<Two_scale>> models{};        ///< running parameters and pole masses
   Spectrum_generator_problems problems{};   ///< spectrum generator problems
   CNMSSMEFTHiggs_scales scales{};              ///< scale information
   CNMSSMEFTHiggs_observables observables{};    ///< observables
};
...
public:
   ...
   void set_input_parameters(const CNMSSMEFTHiggs_input_parameters& input_) { input = input_; }
...
private:
   CNMSSMEFTHiggs_input_parameters input{};     ///< model input parameters
   ...
   std::unique_ptr<CNMSSMEFTHiggs_spectrum> spectrum{nullptr};  ///< spectrum information

   CNMSSMEFTHiggs_slha_io get_slha_io() const;
...
void Model_data::check_spectrum_pointer() const
{
   if (!spectrum) {
      throw SetupError("No spectrum generator set! "
                       "Did you run FSCNMSSMEFTHiggsCalculateSpectrum[]?");
   }
}
                ...
                const std::vector<std::string>& heads = {})
{
   for (std::size_t i = 0; i < flags.size(); i++) {
      if (flags[i]) {
         ...
         MLPutSymbol(link, CNMSSMEFTHiggs_info::particle_names[i].c_str());
      }
   }
}
...
CNMSSMEFTHiggs_slha_io Model_data::get_slha_io() const
{
   check_spectrum_pointer();
   CNMSSMEFTHiggs_slha_io slha_io;
...
void put_spectrum(const CNMSSMEFTHiggs_slha<Model>& model, MLINK link)
{
   MLPutRule(link, CNMSSMEFTHiggs_info::model_name);
...
template <typename Solver_type>
void CNMSSMEFTHiggs_spectrum_impl<Solver_type>::put_model_spectra(MLINK link) const
{
...
template <typename Solver_type>
void CNMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_spectrum(
...
template <typename Solver_type>
void CNMSSMEFTHiggs_spectrum_impl<Solver_type>::calculate_model_observables(
...
template <typename Solver_type>
void CNMSSMEFTHiggs_spectrum_impl<Solver_type>::fill_slha_io(CNMSSMEFTHiggs_slha_io& slha_io) const
{
...
{
   check_spectrum_pointer();
   CNMSSMEFTHiggs_observables observables = spectrum->get_observables();
...
void Model_data::check_spectrum(MLINK link) const
{
   ...
   for (const auto& s: problems.get_problem_strings())
      put_message(link, "FSCNMSSMEFTHiggsCalculateSpectrum", "error", s);

   for (const auto& s: problems.get_warning_strings())
      put_message(link, "FSCNMSSMEFTHiggsCalculateSpectrum", "warning", s);
...
#ifdef ENABLE_SEMI_ANALYTIC_SOLVER
   case 2:
      spectrum.reset(new CNMSSMEFTHiggs_spectrum_impl<Semi_analytic>());
...
DLLEXPORT int FSCNMSSMEFTHiggsGetSettings(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsGetSettings"))
...
DLLEXPORT int FSCNMSSMEFTHiggsGetSMInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsGetSMInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsGetInputParameters(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsGetInputParameters"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsOpenHandle(
...
DLLEXPORT int FSCNMSSMEFTHiggsCloseHandle(
   WolframLibraryData /* libData */, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSCNMSSMEFTHiggsSet(
   WolframLibraryData libData, mint Argc, MArgument* Args, MArgument /* Res */)
{
   using namespace flexiblesusy;
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;
...
DLLEXPORT int FSCNMSSMEFTHiggsGetProblems(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsGetProblems"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsToSLHA(WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsToSLHA"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsGetWarnings(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsGetWarnings"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsCalculateSpectrum(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsCalculateSpectrum"))
      return LIBRARY_TYPE_ERROR;
...
DLLEXPORT int FSCNMSSMEFTHiggsCalculateObservables(
   WolframLibraryData /* libData */, MLINK link)
{
   using namespace flexiblesusy::CNMSSMEFTHiggs_librarylink;

   if (!check_number_of_args(link, 1, "FSCNMSSMEFTHiggsCalculateObservables"))
      return LIBRARY_TYPE_ERROR;
   ...
   try {
      ...
      if (data.get_model_scale() == 0.) {
         put_message(link,
            "FSCNMSSMEFTHiggsCalculateObservables", "warning",
            "Renormalization scale is 0.  Did you run "
            "FSCNMSSMEFTHiggsCalculateSpectrum[]?");
      }
      ...
      put_message(link, "FSCNMSSMEFTHiggsCalculateObservables", "error", e.what());
      put_error_output(link);
   }
