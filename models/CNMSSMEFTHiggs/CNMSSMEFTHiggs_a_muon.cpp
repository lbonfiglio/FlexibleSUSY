...
/**
 * @file CNMSSMEFTHiggs_a_muon.cpp
 *
 * This file was generated at Thu 21 Mar 2019 14:21:51 with FlexibleSUSY
...
#include "cxx_qft/CNMSSMEFTHiggs_qft.hpp"
...
using namespace CNMSSMEFTHiggs_cxx_diagrams;
...
double get_MSUSY(const CNMSSMEFTHiggs_mass_eigenstates& model)
{
...
void run_to_MSUSY(CNMSSMEFTHiggs_mass_eigenstates& model)
{
   ...
   VERBOSE_MSG("CNMSSMEFTHiggs_a_muon: iterate to run to MSUSY ...");
   ...
   do {
      ...
      VERBOSE_MSG("CNMSSMEFTHiggs_a_muon:    running to new MSUSY = " << MSUSY_new);
   ...
   VERBOSE_MSG("CNMSSMEFTHiggs_a_muon: iteration finished. MSUSY = " << MSUSY_new);

   if (precision > precision_goal) {
      WARNING("CNMSSMEFTHiggs_a_muon: run_to_MSUSY(): "
              "Iteration did not converge after " << Nmax
              << " iterations (precision goal: " << precision_goal << ")");
   }
}

double calculate_a_muon_impl(const CNMSSMEFTHiggs_mass_eigenstates& model)
{
   VERBOSE_MSG("CNMSSMEFTHiggs_a_muon: calculating a_mu at Q = " << model.get_scale());

   context_base context{ model };
   double val = 0.0;
   
   using namespace CNMSSMEFTHiggs_cxx_diagrams::fields;

   std::array<int, 1> indices = { 1 };

   val += 1 * AMuonVertexCorrectionFS<typename bar<Fe>::type, Ah>::value(indices, context);
   val += 1 * AMuonVertexCorrectionSF<typename conj<Se>::type, Chi>::value(indices, context);
   val += 1 * AMuonVertexCorrectionFS<typename bar<Fe>::type, hh>::value(indices, context);
   val += 1 * AMuonVertexCorrectionSF<typename conj<Hpm>::type, typename bar<Fv>::type>::value(indices, context);
   val += 1 * AMuonVertexCorrectionFS<typename bar<Cha>::type, typename conj<Sv>::type>::value(indices, context);
...
std::pair<double,double> vary_scale(const CNMSSMEFTHiggs_mass_eigenstates& model)
{
   ...
   std::transform(scales.begin(), scales.end(), scales.begin(),
                  [&model] (double scale) {
                     ...
                     try {
                        ...
                        ERROR("CNMSSMEFTHiggs_a_muon: scale variation: " << e.what());
                     }
                     return amu;
                  });
...
double CNMSSMEFTHiggs_a_muon::calculate_a_muon(const CNMSSMEFTHiggs_mass_eigenstates& model_)
{
   CNMSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("CNMSSMEFTHiggs_a_muon: starting calculation of a_mu ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CNMSSMEFTHiggs_a_muon:" << e.what());
...
double CNMSSMEFTHiggs_a_muon::calculate_a_muon_uncertainty(const CNMSSMEFTHiggs_mass_eigenstates& model_)
{
   CNMSSMEFTHiggs_mass_eigenstates model(model_);

   VERBOSE_MSG("CNMSSMEFTHiggs_a_muon: starting calculation of a_mu uncertainty ...");

   try {
      run_to_MSUSY(model);
      model.get_physical().clear();
   } catch (const Error& e) {
      ERROR("CNMSSMEFTHiggs_a_muon uncertainty: " << e.what());
      return std::numeric_limits<double>::quiet_NaN();
   }
