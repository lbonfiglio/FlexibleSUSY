...
#ifndef CNMSSMEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H
#define CNMSSMEFTHiggs_SPECTRUM_GENERATOR_INTERFACE_H

#include "CNMSSMEFTHiggs_mass_eigenstates.hpp"
#include "CNMSSMEFTHiggs_model.hpp"
#include "CNMSSMEFTHiggs_model_slha.hpp"
#include "CNMSSMEFTHiggs_utilities.hpp"
...
template <class T, class Effective_theory = standard_model::StandardModel<T> >
class CNMSSMEFTHiggs_spectrum_generator_interface {
public:
   virtual ~CNMSSMEFTHiggs_spectrum_generator_interface() = default;

   std::tuple<CNMSSMEFTHiggs<T>, Effective_theory> get_models() const
   { return std::make_tuple(model, eft); }
   std::tuple<CNMSSMEFTHiggs_slha<CNMSSMEFTHiggs<T>>, Effective_theory> get_models_slha() const
   { return std::make_tuple(CNMSSMEFTHiggs_slha<CNMSSMEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.), eft); }

   const CNMSSMEFTHiggs<T>& get_model() const
   { return model; }
   CNMSSMEFTHiggs<T>& get_model()
   { return model; }
   CNMSSMEFTHiggs_slha<CNMSSMEFTHiggs<T>> get_model_slha() const
   { return CNMSSMEFTHiggs_slha<CNMSSMEFTHiggs<T>>(model, settings.get(Spectrum_generator_settings::force_positive_masses) == 0.); }
   ...
   void run(const softsusy::QedQcd&, const CNMSSMEFTHiggs_input_parameters&);
   void write_running_couplings(const std::string& filename, double, double) const;
   void write_spectrum(const std::string& filename = "CNMSSMEFTHiggs_spectrum.dat") const;

protected:
   CNMSSMEFTHiggs<T> model;
   Effective_theory eft{};
   ...
   void translate_exception_to_problem(CNMSSMEFTHiggs<T>& model);
   virtual void run_except(const softsusy::QedQcd&, const CNMSSMEFTHiggs_input_parameters&) = 0;
};
 ...
 */
template <class T, class Effective_theory>
void CNMSSMEFTHiggs_spectrum_generator_interface<T, Effective_theory>::set_settings(
 ...
 */
template <class T, class Effective_theory>
void CNMSSMEFTHiggs_spectrum_generator_interface<T, Effective_theory>::run(
   const softsusy::QedQcd& qedqcd_, const CNMSSMEFTHiggs_input_parameters& input)
{
 ...
 */
template <class T, class Effective_theory>
void CNMSSMEFTHiggs_spectrum_generator_interface<T, Effective_theory>::write_running_couplings(
   const std::string& filename,
   double start, double stop) const
{
   CNMSSMEFTHiggs_mass_eigenstates tmp_model(model);
   try {
   ...
   CNMSSMEFTHiggs_parameter_getter parameter_getter;
   Coupling_monitor<CNMSSMEFTHiggs_mass_eigenstates, CNMSSMEFTHiggs_parameter_getter>
 ...
 */
template <class T, class Effective_theory>
void CNMSSMEFTHiggs_spectrum_generator_interface<T, Effective_theory>::write_spectrum(
   const std::string& filename) const
{
   CNMSSMEFTHiggs_spectrum_plotter plotter(model);
 ...
 */
template <class T, class Effective_theory>
void CNMSSMEFTHiggs_spectrum_generator_interface<T, Effective_theory>::translate_exception_to_problem(
   CNMSSMEFTHiggs<T>& model)
{
   try {
      throw;
