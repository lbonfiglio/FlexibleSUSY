                        ...
                        const std::string& symbol, double scale)
{
   ...
   for (int i = 1; i <= NRows; ++i) {
      ss << format_vector(i, Re(matrix(i-1,0)), "Re(" + symbol + "(" + ToString(i) + "))");
   }
                        ...
                        const std::string& symbol, double scale)
{
   ...
   for (int i = 1; i <= NRows; ++i) {
      for (int k = 1; k <= NCols; ++k) {
         ss << format_matrix(i, k, Re(matrix(i-1,k-1)),
                             ...
                             const std::string& symbol, double scale)
{
   ...
   for (int i = 1; i <= NRows; ++i) {
      ss << format_vector(i, Im(matrix(i-1,0)), "Im(" + symbol + "(" + ToString(i) + "))");
   }
                             ...
                             const std::string& symbol, double scale)
{
   ...
   for (int i = 1; i <= NRows; ++i) {
      for (int k = 1; k <= NCols; ++k) {
         ss << format_matrix(i, k, Im(matrix(i-1,k-1)),
   ...
   for (int i = 1; i <= rows; ++i) {
      if (cols == 1) {
         ...
         for (int k = 1; k <= cols; ++k) {
            ss << format_matrix(i, k, matrix(i-1,k-1),
